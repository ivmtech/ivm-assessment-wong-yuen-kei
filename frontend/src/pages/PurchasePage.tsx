    ...
    const filteredAndSortedPurchases = useMemo(() => {
        ...
        if (dateFilter !== 'all') {
            ...
            filtered = filtered.filter(p => {
                const t = Date.parse(p.purchaseTime);
        ...
        if (searchProduct) {
            ...
            const needle = searchProduct.trim().toLowerCase();
            filtered = filtered.filter(p => {
        ...
        filtered.sort((a, b) => {
            let cmp = 0;
            if (sortField === 'date') {
                const ta = Date.parse(a.purchaseTime) || 0;
                const tb = Date.parse(b.purchaseTime) || 0;
                cmp = ta - tb;
        ...
        return filtered;
    }, [purchases, searchProduct, dateFilter, sortField, sortOrder]);
