...
export default function ProductsPage() {
    ...
    async function buyProduct(productId: string) {
        ...
        // implement validation and purchase flow below (keeps original comments)
        const rawQty = quantities[productId] ?? 1;
        const quantity = Math.floor(Number(rawQty) || 0);

        // validate quantity
        if (quantity <= 0) {
            setMessage('❌ Invalid quantity. Please select at least 1 item.');
        ...
        if (quantity > product.stock) {
            setMessage(`❌ Not enough stock. Only ${product.stock} item(s) available.`);
            setPurchasing({ ...purchasing, [productId]: false });
            return;
        }

        // check machine started
        if (balance === null) {
        ...
        try {
            ...
            if (res.data.success) {
                ...
                await loadProducts();
                const totalCost = typeof res.data.totalCost === 'number' ? res.data.totalCost : cost;
                setBalance(prev => (typeof prev === 'number' ? +(prev - totalCost).toFixed(2) : prev));
    ...
    function updateQuantity(productId: string, newQty: number) {
        ...
        // implement update logic while keeping original comment
        const product = products.find(p => p.id === productId);
        const qty = Math.max(0, Math.floor(Number(newQty) || 0));

        if (!product) {
            // keep behaviour safe for placeholder products
        ...
        if (qty > product.stock) {
            setMessage(`❌ Only ${product.stock} item(s) available. Adjusted to available stock.`);
